...
const getImages = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Fallo al cargar las imágenes." });
  }
};
...
const createNewPost = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "No fue posible crear un nuevo post. Inténtalo de nuevo.",
    });
  }
};
...
const getImage = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Fallo al recuperar la imagen" });
  }
};
